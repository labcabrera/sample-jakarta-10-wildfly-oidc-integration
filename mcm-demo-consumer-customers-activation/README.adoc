...
* _customer-viewer_
* _customer-manager_
* _customer-processor_ (requerido para la llamada de actualización del estado del cliente)
...
Este componente simula la siguiente lógica de negocio a partir del email recibido:

* Si el email contiene la expresión 'exception' lanza una excepción para simular un error en el procesamiento.
* Si el email contiene la expresión 'error' realiza una actualización de estado a 'inactive'
* En caso contrario realiza una actualización de estado a 'active'.
